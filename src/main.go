...
func main() {
	...
	case 1:
		...
		default:
			fmt.Println("Usage: 'goalias [--v?] [help|list(defs)|debug|regcheck]' or 'goalias [--v?] [set|reset|unset|check] [aliasname|!] [alias?]'")
		}
	case 2:
		switch strings.ToLower(os.Args[0]) {
		case "check":
			...
			if registered(os.Args[1]) {
				...
				if strings.Contains(tmp, "cd ") {
					if cdhack {
						...
						os.Args[2] = func(out string) string {
							// tell the user they need to use 'source' or '.'
							out += "if [ \"${BASH_SOURCE[0]}\" == \"${0}\" ]; then"
